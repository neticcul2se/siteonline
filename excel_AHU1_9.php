...
$types  = mysqli_real_escape_string($conn, $_REQUEST['select_visy']);
    ...
    $sql = "SELECT * FROM AHU1_9 where addtime	 ='$dates' and type='$types'";
    $result = $conn->query($sql);
        ...
        $sql = "SELECT * FROM AHU1_9 where addtime like '$strdate' and type='$types'";
        $result = $conn->query($sql);
    }
...
if($md=='date'){
    $newDate = str_replace('/', '-', $dates );
$filename = 'AHU1_9_'.$newDate;
}else{
    $filename = 'AHU1_9_month_'.$select_month;
